...
import { EXERCISES, TRACKS } from '../content/exercises.ts';
import type { Track } from '../content/types.ts';
import type { Attempt, ReviewItem, TrackProgress } from '../data/model.ts';
...
export function initialProgress(): TrackProgress[] {
  return TRACKS.map((t) => ({
    track: t.id,
    level: 1,
    streakAtLevel: 0,
...
export function levelFor(progress: TrackProgress[], track: Track): number {
  return progress.find((p) => p.track === track)?.level ?? 1;
}

function maxLevel(track: Track): number {
  return TRACKS.find((t) => t.id === track)?.maxLevel ?? 1;
}

export interface ProgressionUpdate {
  progress: TrackProgress[];
  /** Trilhas que subiram de nivel nesta sessao — a UI comemora. */
  leveledUp: Array<{ track: Track; level: number }>;
}
...
export function applySessionResults(
  ...
  for (const a of attempts) {
    ...
    if (!ex || a.score < 0) continue;              // nao pontuavel: ignorado
    if (ex.selfReport || ex.rubric === 'guiado') continue;
    const list = byTrack.get(ex.track) ?? [];
    list.push(a.score);
    byTrack.set(ex.track, list);
  }

  const leveledUp: ProgressionUpdate['leveledUp'] = [];
  const next = progress.map((p) => ({ ...p }));

  for (const [track, scores] of byTrack) {
    const row = next.find((p) => p.track === track);
    if (!row || scores.length === 0) continue;
 ...
 */
export function updateReviews(reviews: ReviewItem[], attempts: Attempt[]): ReviewItem[] {
  ...
  for (const a of attempts) {
    ...
    if (!existing) continue;                        // acertou e nao estava na fila
...
export function dueReviews(reviews: ReviewItem[], at = new Date()): ReviewItem[] {
