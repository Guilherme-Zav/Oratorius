...
export interface LineChartOptions {
  height?: number;
  /** Fixa o minimo do eixo Y. Sem isso, uma serie estavel vira uma linha ruidosa ampliada. */
  yMin?: number;
  yMax?: number;
  /** Faixa alvo desenhada ao fundo. */
  band?: { from: number; to: number };
  unit?: string;
  /** Para metricas em que menor e melhor (CV do DDK, alongamentos). */
  lowerIsBetter?: boolean;
}

const W = 320;

export function lineChart(points: SeriesPoint[], options: LineChartOptions = {}): SVGElement {
...
export function streakBars(days: Array<{ day: string; value: number }>): SVGElement {
  const height = 46;
...
/**
 * Waveform com o contorno de f0 sobreposto.
 *
 * As duas curvas juntas sao o que faz o feedback ser lido de relance: o envelope
 * mostra ONDE voce falou e o contorno mostra COMO. Pausas e alongamentos entram
 * como faixas ao fundo.
 */
export interface WaveformOptions {
  waveform: Float32Array;        // pares min/max
  f0: Float32Array;
...
export function waveformChart(opts: WaveformOptions): SVGElement {
  const height = 132;
  const mid = height / 2;
  const root = svg('svg', {
  ...
  const voiced: number[] = [];
  for (let i = 0; i < opts.f0.length; i++) if (opts.f0[i] > 0) voiced.push(opts.f0[i]);
  if (voiced.length > 3) {
    ...
    for (let i = 0; i < opts.f0.length; i++) {
      if (opts.f0[i] <= 0) { pen = false; continue; }
