...
import { mean } from './frames.ts';
import type { PitchTrack } from './pitch.ts';

export interface VoiceQuality {
  /** Variacao relativa media do periodo entre ciclos consecutivos (%). */
  jitterPct: number;
  /** Variacao relativa media da amplitude entre ciclos consecutivos (%). */
  shimmerPct: number;
  /** Razao harmonico-ruido aproximada, em dB. */
  hnrDb: number;
  /** Numero de ciclos usados — abaixo de ~30 o resultado nao e confiavel. */
  cycles: number;
  reliable: boolean;
}
 ...
 */
function longestVoicedRun(pitch: PitchTrack): [number, number] | null {
...
export function analyzeVoiceQuality(
  ...
  if (cycles < 10) return { ...EMPTY, cycles };

  // Jitter local relativo.
  let jitterSum = 0;
  for (let i = 1; i < cycles; i++) jitterSum += Math.abs(periods[i] - periods[i - 1]);
  ...
  for (let i = 1; i < cycles; i++) {
    if (amplitudes[i] <= 1e-9 || amplitudes[i - 1] <= 1e-9) continue;
...
function estimateHnr(x: Float32Array, start: number, end: number, period: number): number {
