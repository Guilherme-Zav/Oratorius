...
export interface PitchOptions {
  fmin: number;
  fmax: number;
  /** Limiar do YIN. Menor = mais exigente, mais frames marcados como nao-vozeados. */
  threshold: number;
  frameMs: number;
  hopMs: number;
  /** Frames abaixo deste RMS nunca sao considerados vozeados. */
  silenceRms: number;
}
...
export interface PitchTrack {
  /** f0 em Hz por frame; 0 = nao vozeado. */
  f0: Float32Array;
  /** Periodo em amostras (interpolado); 0 = nao vozeado. */
  period: Float32Array;
  /** 1 - d'(tau) no minimo escolhido. Proxy de confianca (0..1). */
...
export function yinPeriod(
  ...
  if (tauMax <= tauMin) return { period: 0, confidence: 0 };

  const W = frame.length - tauMax;
  if (W < tauMin) return { period: 0, confidence: 0 };
  ...
  for (let tau = 1; tau <= tauMax; tau++) {
    ...
    if (candidate < 0) {
      if (cmnd[tau] < opts.threshold) candidate = tau;
  ...
  if (tauEstimate < 0) {
    let best = tauMin;
    for (let tau = tauMin + 1; tau <= lastTau; tau++) if (cmnd[tau] < cmnd[best]) best = tau;
    if (cmnd[best] > 0.55) return { period: 0, confidence: 0 };
    tauEstimate = best;
  }
  ...
  if (tauEstimate > 0 && tauEstimate < tauMax) {
    ...
    if (Math.abs(denom) > 1e-12) period = tauEstimate + (c - a) / denom;
  }

  return { period, confidence: Math.max(0, Math.min(1, 1 - cmnd[tauEstimate])) };
}

function frameRms(x: Float32Array, start: number, len: number): number {
  let s = 0;
...
export function trackPitch(
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (frameRms(x, start, frameLen) < opts.silenceRms) continue;
 ...
 */
export function fixOctaveJumps(track: PitchTrack): PitchTrack {
  ...
  for (let i = 1; i < f0.length - 1; i++) {
    ...
    if (ratio > 1.8 && ratio < 2.2) f0[i] = cur / 2;
    else if (ratio > 0.45 && ratio < 0.55) f0[i] = cur * 2;
  }
  return { ...track, f0 };
}

/** Apenas os frames vozeados, para estatistica. */
export function voicedValues(f0: Float32Array): number[] {
  const out: number[] = [];
  for (let i = 0; i < f0.length; i++) if (f0[i] > 0) out.push(f0[i]);
  return out;
}
