...
import { mean, medianFilter, stdDev } from './frames.ts';
import type { SpectralTrack } from './spectrum.ts';
import type { IntensityTrack } from './intensity.ts';

export interface DdkResult {
  /** Instantes (s) dos ataques detectados. */
  onsets: number[];
  count: number;
  /** Silabas por segundo, medido entre o primeiro e o ultimo ataque. */
  syllPerSec: number;
  /** Coeficiente de variacao dos intervalos entre ataques (%). Menor = mais regular. */
  cvPercent: number;
  meanIoiMs: number;
  /** Intervalos individuais, para o grafico de regularidade. */
  ioiMs: number[];
}

export const EMPTY_DDK: DdkResult = {
  onsets: [], count: 0, syllPerSec: 0, cvPercent: 0, meanIoiMs: 0, ioiMs: [],
};

export interface DdkOptions {
  /** Intervalo minimo entre ataques (s). 90 ms ~ 11 silabas/s, acima do humano. */
  minIntervalSec: number;
  /** Multiplicador sobre o limiar adaptativo. Maior = menos ataques detectados. */
  sensitivity: number;
}

export const DEFAULT_DDK_OPTIONS: DdkOptions = {
  minIntervalSec: 0.09,
  sensitivity: 1.5,
};

export function analyzeDdk(
  spectral: SpectralTrack,
  intensity: IntensityTrack,
  options: Partial<DdkOptions> = {},
): DdkResult {
  const opts = { ...DEFAULT_DDK_OPTIONS, ...options };
  const n = Math.min(spectral.flux.length, intensity.rms.length);
  if (n < 4) return EMPTY_DDK;
  ...
  if (onsets.length < 2) return { ...EMPTY_DDK, onsets, count: onsets.length };

  const ioiMs: number[] = [];
  for (let i = 1; i < onsets.length; i++) ioiMs.push((onsets[i] - onsets[i - 1]) * 1000);
  const m = mean(ioiMs);
