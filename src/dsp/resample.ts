...
function lowpass(sampleRate: number, freq: number, q: number): Biquad {
...
export function lowpassFilter(x: Float32Array, sampleRate: number, cutoffHz: number): Float32Array {
  const stages = [lowpass(sampleRate, cutoffHz, 0.5412), lowpass(sampleRate, cutoffHz, 1.3066)];
  let y = x;
  for (const s of stages) y = applyBiquad(y, s);
  // Passagem reversa: cancela o atraso de fase, importante para nao deslocar
  // as fronteiras de silabas e pausas no tempo.
  y = y.slice().reverse() as Float32Array;
  for (const s of stages) y = applyBiquad(y, s);
  return y.slice().reverse() as Float32Array;
}
