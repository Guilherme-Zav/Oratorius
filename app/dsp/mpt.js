 ...
 * trilha D do app.
 */
import { mean, semitones, stdDev } from "./frames.js";
export const EMPTY_MPT = {
 ...
 */
export function analyzeMpt(pitch, intensity, maxGapSec = 0.12) {
    ...
    for (let i = 0; i < intensity.times.length; i++) {
        if (intensity.times[i] >= startSec && intensity.times[i] <= endSec)
            inRange.push(intensity.db[i]);
    }
    let decayDb = 0;
    if (inRange.length >= 6) {
    ...
    if (inRange.length >= 6) {
        ...
        let i = 0;
        while (i < inRange.length && inRange[i] > reference - 6)
            i++;
