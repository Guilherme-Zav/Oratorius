 ...
 */
import { semitones, stdDev } from "./frames.js";
export const DEFAULT_FILLED_PAUSE_OPTIONS = {
    minDurationSec: 0.35,
    maxF0SdSemitones: 1.2,
    maxFlux: 0.13,
};
export function detectFilledPauses(pitch, spectral, options = {}) {
    const opts = { ...DEFAULT_FILLED_PAUSE_OPTIONS, ...options };
    const out = [];
    if (pitch.f0.length === 0)
        return out;
    ...
    for (const [a, b] of runs) {
        ...
        if (fluxN === 0)
            continue;
        if (fluxSum / fluxN > opts.maxFlux)
            continue;
        out.push({ startSec, endSec, durationSec });
    }
    return out;
}
//# sourceMappingURL=filledPause.js.map