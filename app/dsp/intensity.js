...
export function trackIntensity(x, sampleRate, frameMs = 25, hopMs = 10) {
...
export function clippingRatio(x, threshold = 0.995) {
    if (x.length === 0)
        return 0;
    let n = 0;
    for (let i = 0; i < x.length; i++)
 ...
 */
export function estimateNoiseFloorDb(track) {
    if (track.db.length === 0)
        return DB_FLOOR;
