 ...
 */
export const DEFAULT_SYLLABLE_OPTIONS = {
    dipDb: 2,
    minDistanceSec: 0.1,
    smoothFrames: 5,
};
...
function isVoicedAt(pitch, timeSec, toleranceSec = 0.03) {
    if (!pitch || pitch.f0.length === 0)
        return true; // sem pista de pitch, nao filtra
    for (let i = 0; i < pitch.f0.length; i++) {
        if (Math.abs(pitch.times[i] - timeSec) <= toleranceSec && pitch.f0[i] > 0)
            return true;
    }
    return false;
}
export function detectSyllables(intensity, pitch, thresholdDb, totalSec, speechSegments, options = {}) {
    ...
    for (const p of peaks) {
        ...
        let valley = Infinity;
        for (let i = prev; i <= p; i++)
            valley = Math.min(valley, env[i]);
