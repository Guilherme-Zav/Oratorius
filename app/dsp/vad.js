 ...
 */
import { estimateNoiseFloorDb } from "./intensity.js";
export const DEFAULT_VAD_OPTIONS = {
    marginDb: 9,
    minPauseSec: 0.25,
    minSpeechSec: 0.08,
};
function mergeShortGaps(segments, minGap) {
...
export function detectVoiceActivity(intensity, pitch, totalSec, options = {}) {
    ...
    if (pitch && pitch.f0.length > 0) {
        ...
        for (let i = 0; i < pitch.f0.length; i++) {
            ...
            if (!speech.some((s) => t >= s.startSec && t <= s.endSec))
                continue;
            inSpeech++;
            if (pitch.f0[i] > 0)
                voiced++;
