 ...
 */
import { median, percentile, removeDC, semitones, stdDev, mean } from "./frames.js";
import { ANALYSIS_RATE, resample } from "./resample.js";
import { fixOctaveJumps, trackPitch, voicedValues } from "./pitch.js";
import { clippingRatio, peakDb, trackIntensity } from "./intensity.js";
import { detectVoiceActivity } from "./vad.js";
import { analyzeSpectrum } from "./spectrum.js";
import { detectSyllables } from "./syllables.js";
import { detectFilledPauses } from "./filledPause.js";
import { analyzeVoiceQuality } from "./voiceQuality.js";
import { analyzeDdk, EMPTY_DDK } from "./ddk.js";
import { analyzeMpt } from "./mpt.js";
/** Envelope min/max reduzido — desenhar 480k amostras num canvas de 350 px e desperdicio. */
function buildWaveform(x, bins) {
...
export function analyze(input, inputRate, options = {}) {
    ...
    const syllables = detectSyllables(intensity, pitch, vad.thresholdDb, totalSec, vad.speech);
    ...
    for (let i = 0; i < intensity.times.length; i++) {
        ...
        if (vad.speech.some((s) => t >= s.startSec && t <= s.endSec))
            speechDb.push(intensity.db[i]);
    }
    ...
    if (snrDb < 15)
        warnings.push('ruido-alto');
    if (totalSec < 0.4)
        warnings.push('muito-curto');
    if (vad.speechTotalSec < 0.2 || voiced.length < 5)
        warnings.push('sem-voz');
    if (peak < -35)
        warnings.push('volume-baixo');
