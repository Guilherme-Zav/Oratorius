...
import { allAttempts, dailyStats } from "../../data/store.js";
...
const METRICS = [
    ...
    },
    {
        ...
        explanation: 'Variacao dos intervalos entre silabas. E a metrica mais sensivel a travamento — e a unica em que MENOR e melhor.',
        unit: '%',
        lowerIsBetter: true,
        exerciseIds: ['ddk-pataka'],
    ...
    },
    {
        ...
        explanation: 'Desvio do contorno de f0 em semitons na leitura expressiva. Abaixo de ~1,5 a fala soa monotona.',
        unit: ' st',
        band: { from: 2, to: 6 },
    ...
    },
    {
        ...
        explanation: 'Quantos "eeee" e "hmmm" por minuto de fala. Detectado pelo som, sem transcricao — nao pega "tipo" nem "ne".',
        unit: '/min',
        lowerIsBetter: true,
    ...
    },
    {
        ...
        explanation: 'Desempenho em prato/pato, trem/tem e afins — onde a omissao do R aparece com clareza.',
        unit: '',
        band: { from: 85, to: 100 },
...
async function hydrate(ctx, root) {
    ...
    if (attempts.length === 0) {
        root.appendChild(h('div', { class: 'notice info' }, h('strong', { text: 'Ainda sem dados' }), h('p', {
    ...
    const meanScore = scored.length
        ...
        : 0;
    root.appendChild(h('section', { class: 'card' }, h('div', { class: 'stat-row' }, h('div', { class: 'stat' }, h('span', { class: 'stat-value', text: String(attempts.length) }), h('span', { class: 'stat-label', text: 'gravacoes' })), h('div', { class: 'stat' }, h('span', { class: 'stat-value', text: fmtDuration(totalSec) }), h('span', { class: 'stat-label', text: 'de treino' })), h('div', { class: 'stat' }, h('span', { class: 'stat-value', text: String(Math.round(meanScore)) }), h('span', { class: 'stat-label', text: 'nota media' }))), lineChart(stats.map((s) => ({ label: s.day, value: s.practiceSec > 0 ? s.practiceSec / 60 : null })), { height: 90, unit: ' min', yMin: 0 }), h('p', { class: 'muted small', text: 'minutos por dia, ultimos 30 dias' })));
    // ------------------------------------------------ uma serie por metrica
    for (const spec of METRICS) {
        const series = buildSeries(attempts, spec);
        const withData = series.filter((p) => p.value !== null);
        if (withData.length < 2)
            continue; // uma medicao nao e tendencia
        ...
        root.appendChild(h('section', { class: 'card metric-card' }, h('div', { class: 'metric-head' }, h('h2', { text: spec.title }), h('span', {
        ...
        }), h('p', { class: 'muted small', text: spec.explanation }), h('p', { class: 'muted small', text: `${withData.length} medicoes · atual: ${spec.format(last)}` })));
    }
    // ------------------------------------------------ exportar relatorio
    ...
    root.appendChild(h('section', { class: 'card' }, h('h2', { text: 'Levar a um profissional' }), h('p', { class: 'muted small' }, 'Um fonoaudiologo reconhece TMF, DDK e taxa de acerto por contexto. ', 'Este relatorio junta essas medidas em texto simples.'), exportBtn));
    root.appendChild(h('p', { class: 'disclaimer' }, 'O Oratorius nao e dispositivo medico e nao diagnostica lingua presa (anquiloglossia). ', 'Se houver causa anatomica, exercicio nenhum resolve — a avaliacao e clinica.'));
}
/**
 ...
 */
function buildSeries(attempts, spec) {
    const relevant = attempts.filter((a) => {
        if (spec.exerciseIds && !spec.exerciseIds.includes(a.exerciseId))
            return false;
    ...
    for (const a of relevant) {
        ...
        if (cur === undefined)
            ...
            byDay.set(day, spec.lowerIsBetter ? Math.min(cur, v) : Math.max(cur, v));
    }
...
function downloadReport(ctx, attempts) {
    const lines = [];
    ...
    for (const spec of METRICS) {
        ...
        for (const p of series)
            lines.push(`  ${p.label}  ${spec.format(p.value)}`);
        lines.push('');
    }
