 ...
 */
import { h, fmtDuration } from "../dom.js";
import { streakBars } from "../charts.js";
import { registerScreen } from "../app.js";
import { buildSession } from "../../engine/session.js";
import { startPractice } from "./practice.js";
import { currentStreak, dailyStats, daysSinceBackup, practiceSecondsToday, recentAttempts, } from "../../data/store.js";
import { levelFor } from "../../engine/progression.js";
import { TRACKS } from "../../content/exercises.js";
...
export function renderHome(ctx) {
    ...
    root.appendChild(h('header', { class: 'app-header' }, h('h1', { class: 'brand', text: 'Oratorius' }), h('p', { class: 'brand-sub', text: 'treino de fala' })));
    // ------------------------------------------------ acao primaria
    const cta = h('button', { class: 'cta' }, h('span', { class: 'cta-label', text: 'Treinar hoje' }), h('span', {
        class: 'cta-meta',
    ...
    for (const b of plan.blocks) {
        blocks.appendChild(h('li', { class: 'plan-item' }, h('strong', { text: b.title }), h('span', { class: 'muted', text: b.subtitle })));
    }
    ...
    for (const t of TRACKS) {
        ...
        levelList.appendChild(h('li', { class: 'level-row' }, h('span', { text: t.name }), h('span', { class: 'level-badge', text: `${level}/${t.maxLevel}` })));
    }
...
async function hydrate(ctx, statsCard, root) {
    ...
    statsCard.replaceChildren(h('h2', { text: 'Constancia' }), h('div', { class: 'stat-row' }, h('div', { class: 'stat' }, h('span', { class: 'stat-value', text: String(streak) }), h('span', { class: 'stat-label', text: streak === 1 ? 'dia seguido' : 'dias seguidos' })), h('div', { class: 'stat' }, h('span', { class: 'stat-value', text: fmtDuration(todaySec) }), h('span', { class: 'stat-label', text: `de ${ctx.state.settings.dailyGoalMin} min hoje` }))), h('div', { class: 'progress-bar' }, h('div', { class: 'progress-fill', style: `width:${pct}%` })), streakBars(stats.map((s) => ({ day: s.day, value: s.practiceSec }))), h('p', { class: 'muted small', text: 'ultimas 3 semanas' }));
    if (recent.length === 0) {
        root.insertBefore(h('div', { class: 'notice info' }, h('strong', { text: 'Primeira vez?' }), h('p', {
    ...
    if (sinceBackup === null || sinceBackup >= 7) {
        ...
        root.insertBefore(h('div', { class: `notice ${urgent ? 'warn' : 'info'}` }, h('strong', {
