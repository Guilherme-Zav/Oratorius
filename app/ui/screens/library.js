...
import { EXERCISES, TRACKS, exercisesByTrack } from "../../content/exercises.js";
import { levelFor } from "../../engine/progression.js";
import { buildTrackSession, singleExerciseSession } from "../../engine/session.js";
import { startPractice } from "./practice.js";
import { allAttempts } from "../../data/store.js";
let openTrack = null;
export function renderLibrary(ctx) {
    ...
    for (const track of TRACKS) {
        ...
        const header = h('button', { class: `track-head${isOpen ? ' open' : ''}` }, h('div', { class: 'track-info' }, h('strong', { text: track.name }), h('span', { class: 'muted small', text: track.description })), h('span', { class: 'level-badge', text: `${level}/${track.maxLevel}` }));
        ...
        if (isOpen) {
            ...
            for (const ex of list) {
                if (ex.level !== lastLevel) {
                    ...
                    ul.appendChild(h('li', { class: `level-sep${locked ? ' locked' : ''}` }, `Nivel ${ex.level}`, locked ? h('span', { class: 'lock', text: ' · ainda nao liberado' }) : null));
                }
    ...
    root.appendChild(h('p', { class: 'muted small center' }, `${EXERCISES.length} exercicios no catalogo.`));
    void annotateCounts(root);
    return root;
}
function exerciseRow(ctx, ex, locked) {
    const row = h('li', { class: `exercise-row${locked ? ' locked' : ''}` }, h('div', { class: 'exercise-row-main' }, h('strong', { text: ex.title }), h('span', { class: 'muted small', text: preview(ex.prompt) })), h('span', { class: 'count', data: { exercise: ex.id }, text: '' }));
...
async function annotateCounts(root) {
    const attempts = await allAttempts();
    const counts = new Map();
    for (const a of attempts)
        counts.set(a.exerciseId, (counts.get(a.exerciseId) ?? 0) + 1);
    for (const el of root.querySelectorAll('[data-exercise]')) {
