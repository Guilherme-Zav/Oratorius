 ...
 */
import { EXERCISES, baselineExercises, exercisesAtLevel, getExercise, warmupExercises, } from "../content/exercises.js";
import { dueReviews, levelFor } from "./progression.js";
/** Escolha estavel dentro do dia: o mesmo plano se voce reabrir o app. */
function dailyPick(items, seed, count) {
 ...
 */
function atOrBelow(track, level, count, seed) {
    const out = [];
...
export function buildSession(progress, reviews, date = new Date()) {
...
export function buildTrackSession(track, level) {
    const ids = EXERCISES
...
export function singleExerciseSession(exerciseId) {
